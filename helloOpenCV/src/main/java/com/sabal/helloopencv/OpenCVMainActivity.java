...
public class OpenCVMainActivity extends Activity implements CvCameraViewListener2 {
	...
	private int mHistSizeNum = 25, power = 100, pin = 87, real_power = 45,sto=45,minU=45;
	Point p, p1, p2, p3, p4;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Rever.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				...
				if (isChecked) {
					revelog = -1;
					reverer.setTextColor(R.color.GREEN);
				} else {
					revelog = 1;
					reverer.setTextColor(R.color.RED);
				}
			}
		});
//ff
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		if (c1 < 3 && prevU > mOpenCvCameraView.getHeight() / 2)
			c1 = mOpenCvCameraView.getHeight();
		if (c2 < 3 && prevD > mOpenCvCameraView.getHeight() / 2)
